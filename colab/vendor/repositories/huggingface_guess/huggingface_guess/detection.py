...
def detect_unet_config(state_dict, key_prefix):
    ...
    if '{}cap_embedder.0.weight'.format(key_prefix) in state_dict_keys:  # Lumina 2
        unet_config = {}
        unet_config["image_model"] = "lumina2"
        unet_config["sample_size"] = 128
        unet_config["patch_size"] = 2
        unet_config["in_channels"] = 16
        unet_config["hidden_size"] = 2304
        unet_config["num_layers"] = 26
        unet_config["num_refiner_layers"] = 2
        unet_config["num_attention_heads"] = 24
        unet_config["num_kv_heads"] = 8
        unet_config["multiple_of"] = 256
        unet_config["ffn_dim_multiplier"] = None
        unet_config["norm_eps"] = 1e-5
        unet_config["scaling_factor"] = 1.0
        unet_config["axes_dim_rope"] = [32, 32, 32]
        unet_config["axes_lens"] = [300, 512, 512]
        unet_config["cap_feat_dim"] = 2304
        return unet_config

    if (
        '{}blocks.0.mlp.layer1.weight'.format(key_prefix) in state_dict_keys
        or '{}net.blocks.0.mlp.layer1.weight'.format(key_prefix) in state_dict_keys
    ):  # Cosmos predict2 / Anima
        cosmos_prefix = key_prefix
        if '{}blocks.0.mlp.layer1.weight'.format(cosmos_prefix) not in state_dict_keys:
            cosmos_prefix = '{}net.'.format(key_prefix)
        dit_config = {}
        dit_config["image_model"] = "cosmos_predict2"
        if "{}llm_adapter.blocks.0.cross_attn.q_proj.weight".format(cosmos_prefix) in state_dict_keys:
            dit_config["image_model"] = "anima_cosmos"
        dit_config["max_img_h"] = 240
        dit_config["max_img_w"] = 240
        dit_config["max_frames"] = 128
        concat_padding_mask = True
        dit_config["in_channels"] = (state_dict['{}x_embedder.proj.1.weight'.format(cosmos_prefix)].shape[1] // 4) - int(concat_padding_mask)
        dit_config["out_channels"] = 16
        dit_config["patch_spatial"] = 2
        dit_config["patch_temporal"] = 1
        dit_config["model_channels"] = state_dict['{}x_embedder.proj.1.weight'.format(cosmos_prefix)].shape[0]
    ...
    if '{}double_blocks.0.img_attn.norm.key_norm.scale'.format(key_prefix) in state_dict_keys:  # Flux
        ...
        if any(k.startswith("text_encoders.gemma2_2b.") for k in state_dict_keys):
            image_model_name = "anima"

        dit_config = {}
        dit_config["image_model"] = image_model_name
