...
from backend.nn.cosmos_position_embedding import VideoRopePosition3DEmb, LearnablePosEmbAxis
...
class GPT2FeedForward(nn.Module):
    def __init__(self, d_model: int, d_ff: int, device=None, dtype=None, operations=None) -> None:
        super().__init__()
        operations = operations or CosmosOps
...
def torch_attention_op(q_B_S_H_D: torch.Tensor, k_B_S_H_D: torch.Tensor, v_B_S_H_D: torch.Tensor, transformer_options: Optional[dict] = {}) -> torch.Tensor:
    ...
    return attention_function(q_B_H_S_D, k_B_H_S_D, v_B_H_S_D, q_B_H_S_D.shape[1], skip_reshape=True)
