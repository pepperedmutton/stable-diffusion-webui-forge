...
class Attention(nn.Module):
    def __init__(self, query_dim, context_dim, n_heads, head_dim, device=None, dtype=None, operations=None):
        super().__init__()
        operations = operations or AnimaOps
...
class Anima(MiniTrainDIT):
    def __init__(self, *args, **kwargs):
        kwargs.setdefault("operations", AnimaOps)
        super().__init__(*args, **kwargs)
        self.llm_adapter = LLMAdapter(device=kwargs.get("device"), dtype=kwargs.get("dtype"), operations=kwargs.get("operations"))

    def preprocess_text_embeds(self, text_embeds, text_ids, t5xxl_weights=None):
        if text_ids is not None:
            if text_ids.dtype not in (torch.int32, torch.int64):
...
class IntegratedAnimaCosmosTransformer2DModel(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
        kwargs.setdefault("operations", AnimaOps)
        self.inner_model = Anima(**kwargs)

    def preprocess_text_embeds(self, text_embeds, text_ids, t5xxl_weights=None):
        return self.inner_model.preprocess_text_embeds(text_embeds, text_ids, t5xxl_weights=t5xxl_weights)

    def forward(self, x, timestep, context, **kwargs):
        if x.ndim == 4:
            x = x.unsqueeze(2)
            out = self.inner_model(x, timestep, context, **kwargs)
            return out.squeeze(2)
        return self.inner_model(x, timestep, context, **kwargs)
