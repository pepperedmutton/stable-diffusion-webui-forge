...
def _get_model_predictor(model):
    ...
    return getattr(inner, "predictor", None)


def _is_const_predictor(predictor):
    return predictor is not None and getattr(predictor, "prediction_type", None) == "const"


def sigma_to_half_log_snr(sigma, predictor):
    # For flow/const predictors, sigma is t in [0, 1], and half-logSNR is -logit(t).
    if _is_const_predictor(predictor):
        return sigma.logit().neg()
    return sigma.log().neg()


def offset_first_sigma_for_snr(sigmas, predictor, percent_offset=1e-4):
    if len(sigmas) <= 1:
        return sigmas

    if _is_const_predictor(predictor) and sigmas[0] >= 1:
        sigmas = sigmas.clone()
        if hasattr(predictor, "percent_to_sigma"):
            sigmas[0] = predictor.percent_to_sigma(percent_offset)
        else:
            sigmas[0] = 1.0 - percent_offset
...
def sample_euler_ancestral(model, x, sigmas, extra_args=None, callback=None, disable=None, eta=1., s_noise=1., noise_sampler=None):
    ...
    if isinstance(predictor, (PredictionFlux, PredictionDiscreteFlow)) or _is_const_predictor(predictor):
...
def sample_res_multistep(model, x, sigmas, extra_args=None, callback=None, disable=None, s_noise=1., noise_sampler=None):
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigma_down == 0 or old_denoised is None:
            ...
            # Second-order multistep update ported from ComfyUI's Lumina workflow defaults.
            t = t_fn(sigmas[i])
...
def sample_er_sde(model, x, sigmas, extra_args=None, callback=None, disable=None, s_noise=1.0, noise_sampler=None, noise_scaler=None, max_stage=3):
    ...
    def default_er_sde_noise_scaler(y):
        return y * ((y ** 0.3).exp() + 10.0)
    ...
    predictor = _get_model_predictor(model)
    sigmas = offset_first_sigma_for_snr(sigmas, predictor)
    half_log_snrs = sigma_to_half_log_snr(sigmas, predictor)
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigmas[i + 1] == 0:
            ...
            if s_noise > 0:
                noise_term = (er_lambda_t ** 2 - er_lambda_s ** 2 * r ** 2).sqrt().nan_to_num(nan=0.0)
                x = x + alpha_t * noise_sampler(sigmas[i], sigmas[i + 1]) * s_noise * noise_term
        old_denoised = denoised
    return x
